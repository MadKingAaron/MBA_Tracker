...
class Spreadsheet:
    ...
    def setWorksheets(self):
        ...
        self.foundation_courses = {foundation_courses_names[i]: self.FoundationWorksheet(self, foundation_courses_names[i]) for i
    ...
    def copyClipboardToAllWS(self):
        for course in self.foundation_courses:
            self.foundation_courses[course].pasteRange(self.table_startingCol, self.table_startingRow, self.table_endCol, self.table_endRow)
            self.foundation_courses[course].calculateCounts()


        for course in self.core_courses:
            self.core_courses[course].pasteRange(self.table_startingCol, self.table_startingRow, self.table_endCol, self.table_endRow)
            self.core_courses[course].calculateCounts()

        for course in self.special_topics_courses:
            self.special_topics_courses[course].pasteRange(self.table_startingCol, self.table_startingRow, self.table_endCol, self.table_endRow)
    ...
    class Worksheet:
        ...
        def pasteRange(self, startCol, startRow, endCol, endRow):
            ...
            for i in range(startRow, endRow + 1, 1):
                countCol = 0
                for j in range(startCol, endCol + 1, 1):
                    self.current_worksheet.cell(row=i, column=j).value = self.current_spreadsheet.clipboard[countRow][
                        countCol]
                    self.current_spreadsheet.fontClipboard[countRow][countCol].copyToCell(self.current_worksheet.cell(row=i, column=j))
                    # self.current_worksheet.cell(row=i, column=j).font = copy(self.current_spreadsheet.fontClipboard[countRow][countCol])
    ...
    class CoreWorksheet(Worksheet):
        ...
        def calculateCounts(self):
            MBA_subtotal = "=SUBTOTAL(3, D" + str(self.current_spreadsheet.MBA_start_row) + ":D" + str(self.current_spreadsheet.MBA_end_row) + ")"
            self.current_worksheet.cell(row=169, column=4).value = MBA_subtotal

            if "570" not in self.current_worksheet_name:
                DD_subtotal = "=SUBTOTAL(3, D" + str(self.current_spreadsheet.DD_start_row) + ":D" + str(self.current_spreadsheet.DD_end_row) + ")"
    ...
    class FoundationWorksheet(Worksheet):
        ...
        def calculateCounts(self):
            ...
            Bus_Cert_subtotal = "=SUBTOTAL(3, D" + str(self.current_spreadsheet.Bus_Cert_start_row) + ":D" + str(self.current_spreadsheet.Bus_Cert_end_row) + ")"
